...
def generate_synthetic_positions(valuation_date) -> pd.DataFrame:
    data = {
        'PositionID': [f'POS{i:03d}' for i in range(1, 11)],
        'InstrumentType': ['Stock', 'Stock', 'FX Forward', 'FX Forward', 'European Option', 'European Option', 'Stock', 'European Option', 'FX Forward', 'Stock'],
        ...
        'Maturity': [
            pd.NaT, pd.NaT, valuation_date + timedelta(days=90), valuation_date + timedelta(days=180),
            valuation_date + timedelta(days=60), valuation_date + timedelta(days=120), pd.NaT,
            valuation_date + timedelta(days=90), valuation_date + timedelta(days=30), pd.NaT,
        ],
